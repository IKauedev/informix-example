...
public class SimpleyTreen {
    private static final Logger log = LoggerFactory.getLogger(SimpleyTreen.class);

    public static void main(String[] args) {
        SimpleyTreen rr = new SimpleyTreen();
        rr.executeDbOperation("SELECT * FROM items ORDER BY item_num");
        rr.executeDbOperation("SELECT * FROM items WHERE total_price < 50 ORDER BY total_price");
    }
